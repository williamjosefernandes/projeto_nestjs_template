...
7. **`POST /onboarding/complete` devolvia 500 em CPF/CNPJ/telefone duplicado.** `Customer.document`/`Company.document`/`User.phone` são `@unique`; sem checagem prévia, o `PrismaClientKnownRequestError` (P2002) do `tx.customer.create()`/`tx.company.create()`/`tx.user.update()` estourava direto pro `AllExceptionsFilter`, que só sabia mapear pra `500 INTERNAL_SERVER_ERROR` genérico. Encontrado rodando o fluxo completo em navegador (Playwright), não em revisão de código. Corrigido com o mesmo padrão de `AuthService.register()` (checagem prévia dentro da própria transaction + exceção de negócio): `OnboardingService.complete()` ganhou `assertDocumentAvailable`/`assertPhoneAvailable`, lançando `ConflictAppException` com os novos `ErrorCode.DOCUMENT_ALREADY_IN_USE`/`PHONE_ALREADY_IN_USE` (409). Ver `projeto_vite_template/docs/10-onboarding-cadastro.md §4.1`.
...
## 8. Módulo de Onboarding (`src/onboarding` + `src/geo`)

Fase que liga o wizard de `/cadastro` do frontend (Cliente/`Customer` e Empresa/`Company`) ao backend de ponta a ponta — antes, `POST /auth/register` criava só o `User`; agora cria também um `OnboardingDraft` (rascunho por step, `payload: Json`), e `POST /onboarding/complete` cria `Account`+perfis padrão+`Membership` (Owner)+`Customer`\|`Company`+`Address` numa única transaction. Detalhamento completo (schema, endpoints, decisões de persistência, bugs encontrados só em teste de navegador real) em `projeto_vite_template/docs/10-onboarding-cadastro.md` — não duplicado aqui de propósito, esta seção só referencia o que é específico da massa de teste:

- **CPF/CNPJ de teste devem ser gerados com checksum mod-11 válido** (`@IsCPF`/`@IsCNPJ` validam de verdade, não só formato) — CPFs "de brinquedo" tipo `111.111.111-11` são rejeitados. Exemplo de CPF válido usado nos testes E2E: `529.982.247-25`.
- **`User.phone` é `@unique` globalmente** — reaproveitar o mesmo telefone entre dois cadastros de teste diferentes gera `PHONE_ALREADY_IN_USE` (409), não um erro genérico (ver item 7 acima).
- **Provisionamento de perfis é o mesmo dos dois caminhos**: `provisionDefaultProfiles` (`src/core/security/provisioning/`) é chamado tanto pelo script de seed quanto por `OnboardingService.complete()` — uma conta criada via onboarding em runtime recebe exatamente a mesma matriz de perfis/permissões (seção 3) que as contas seedadas.
